...
def convert_json_to_csv(json_data, csv_filename):
    with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # 컬럼명 설정 - 필요한 필드만 선택
        columns = ['title','contentsid','type_value','type_label', 'address', 'roadaddress','postcode', 'phoneno', 'latitude', 'longitude','region1cd_value','region1cd_label','region2cd_value', 'region2cd_label','photoid','imgpath', 'thumbnailpath','introduction','alltags','tag']
          ...
          for item in data['items']:
            try:
                    row = [
                        ...
                        item.get('repPhoto', {}).get('photoid', {}).get('photoid', ''),  # 예외 처리 추가
